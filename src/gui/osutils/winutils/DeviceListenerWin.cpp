                                                ...
                                                const QUuid* deviceClass)
{
    ...
    m_deviceIdPrefix = R"(\\?\USB#)";
    if (vendorId > 0) {
        m_deviceIdPrefix += QString("VID_%1&").arg(vendorId, 0, 16);
        if (productId > 0) {
            m_deviceIdPrefix += QString("PID_%1&").arg(productId, 0, 16);
        }
    }
...
bool DeviceListenerWin::nativeEventFilter(const QByteArray& eventType, void* message, long*)
{
    ...
    if ((m_handleArrival && m->wParam == DBT_DEVICEARRIVAL)
        ...
        if (name.startsWith(m_deviceIdPrefix)) {
